...
# 参数解析
parser = argparse.ArgumentParser()  # 创建一个新的参数解析器,添加命令行参数，包括GPU设备编号、随机种子、数据集、是否测试、嵌入维度、主题数、到达时间模块类型、编码器类型、批次大小和训练周期数
...
# 环境设置
gpu_list = args.gpu
torch.manual_seed(args.seed)    # 设置随机种子以保证结果可复现
...
if __name__ == '__main__':
    ...
    # 配置更新，使用update_config函数更新配置文件，包括主题数、随机种子、到达时间模块类型、嵌入维度、编码器类型、批次大小和训练周期数
    ...
    # 报告文件，使用report.txt文件记录训练批次、周期信息和总运行时间
