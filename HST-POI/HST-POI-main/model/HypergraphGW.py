...
num_pois = len(location2id)   # 假设POI的数量是location2id字典的长度
...
H_poi_src = csr_matrix_drop_edge(H_poi_src, 0.7)
...
H_friend = csr_matrix_drop_edge(H_friend_sparse, 1.0)
Deg_H_friend = get_hyper_deg(H_friend)
HG_friend = Deg_H_friend * H_friend

H_endfri = H_friend.T  # 朋友超图逆
Deg_H_endfri = get_hyper_deg(H_endfri)
HG_endfri = Deg_H_endfri * H_endfri


# 综合图
omiga = 0.2  # 朋友影响权重
H_total = H_up + omiga * H_friend
Deg_H_total = get_hyper_deg(H_total)
HG_total = Deg_H_total * H_total

H_totalrev = H_total.T
Deg_H_totalrev = get_hyper_deg(H_totalrev)
...
w_gate_geo = torch.empty(up_dim, up_dim)
b_gate_geo = torch.empty(1, up_dim)
w_gate_friend = torch.empty(up_dim, up_dim)
b_gate_friend = torch.empty(1, up_dim)
w_gate_total = torch.empty(up_dim, up_dim)
b_gate_total = torch.empty(1, up_dim)
...
nn.init.xavier_normal_(w_gate_geo)
nn.init.xavier_normal_(b_gate_geo)      # 偏置也用Xavier
nn.init.xavier_normal_(w_gate_friend)
nn.init.xavier_normal_(b_gate_friend)
nn.init.xavier_normal_(w_gate_total)
nn.init.xavier_normal_(b_gate_total)      # 偏置也用Xavier
...
gate_friend = torch.sigmoid(raw_emb @ w_gate_friend + b_gate_friend)
poi_emb3_tensor = raw_emb * gate_friend
# poi_emb3_tensor = raw_emb
final_pre_emb3 = [poi_emb3_tensor]

# 一层卷积
pre_emb3 = torch.sparse.mm(HG_friend_tensor, poi_emb3_tensor)  # 超图卷积,源节点->目标节点
pre_emb3 = torch.sparse.mm(HG_endfri_tensor, pre_emb3)
pre_emb3 = pre_emb3 + final_pre_emb3[-1]  # 残差连接
# pre_emb1 = F.relu(pre_emb1)
pre_emb3 = dropout(pre_emb3)
final_pre_emb3.append(pre_emb3)

# 二层卷积
pre_emb3 = torch.sparse.mm(HG_friend_tensor, pre_emb3)  # 超图卷积,源节点->目标节点
pre_emb3 = torch.sparse.mm(HG_endfri_tensor, pre_emb3)
pre_emb3 = pre_emb3 + final_pre_emb3[-1]  # 残差连接
# pre_emb1 = F.relu(pre_emb1)
pre_emb3 = dropout(pre_emb3)
...
final_pre_emb3 = torch.mean(torch.stack(final_pre_emb3), dim=0)  # 取均值
final_user_emb3 = torch.sparse.mm(HG_friend_tensor, final_pre_emb3)

# 综合超图（用户+朋友）
HG_total_tensor = torch.tensor(HG_total.toarray(), dtype=torch.float32)
HG_totalrev_tensor = torch.tensor(HG_totalrev.toarray(), dtype=torch.float32)

gate_total = torch.sigmoid(raw_emb @ w_gate_total + b_gate_total)
poi_emb4_tensor = raw_emb * gate_total
# poi_emb4_tensor = raw_emb
final_pre_emb4 = [poi_emb4_tensor]

# 一层卷积
pre_emb4 = torch.sparse.mm(HG_total_tensor, poi_emb4_tensor)  # 超图卷积,源节点->目标节点
pre_emb4 = torch.sparse.mm(HG_totalrev_tensor, pre_emb4)
pre_emb4 = pre_emb4 + final_pre_emb4[-1]  # 残差连接
# pre_emb1 = F.relu(pre_emb1)
pre_emb4 = dropout(pre_emb4)
final_pre_emb4.append(pre_emb4)

# 二层卷积
pre_emb4 = torch.sparse.mm(HG_total_tensor, pre_emb4)  # 超图卷积,源节点->目标节点
pre_emb4 = torch.sparse.mm(HG_totalrev_tensor, pre_emb4)
pre_emb4 = pre_emb4 + final_pre_emb4[-1]  # 残差连接
pre_emb4 = dropout(pre_emb4)
final_pre_emb4.append(pre_emb4)
...
final_pre_emb4 = torch.mean(torch.stack(final_pre_emb4), dim=0)  # 取均值
final_user_emb4 = torch.sparse.mm(HG_up_tensor, final_pre_emb4)  # 最后传播到自身
